/*==============================================================================
DO FILE NAME:			S4_02a_cr_create_nsaid_population
PROJECT:				NSAIDs in COVID-19 
DATE: 					15 June 2020 
AUTHOR:					A Wong (modified from ICS study by A Schultze)							
DESCRIPTION OF FILE:	program 01, NSAID population (cohort 1) for NSAID project  
						check inclusion/exclusion citeria
						drop patients if not relevant 
DEPENDENCIES: 
DATASETS USED:			data in memory (from analysis/input_nsaid_population.csv)
...
cap log close
log using $logdir\S4_02a_cr_create_nsaid_population, replace t
...
noi di "DROP PATIENTS WITH INDOMETACIN IN THE EXPOSED GROUP"
drop if exposure == 1 & indometacin == 1
...
* EXCLUSION 8: PATIENTS WHO HAD INDOMETACIN
capture confirm string variable indometacin
		if _rc!=0 {
			noi di "indometacin: numeric variable datacheck"
			noi datacheck indometacin==. , nolist
		}
		else {
		    noi di "indometacin: string variable datacheck"
			noi datacheck missing(indometacin), nolist
		}
